...
public class SortingPanel extends JPanel {
	...
	private static final String FILE_NAME = "numbers.txt"; // This String is used by the createArray() function
																	// to get
	// integers from a text file. You can change this value
	// to
	...
	private int mergeComparisons;
	private int selectionComparisons;
	private int insertionComparisons;// bubble sort required. You'll need to add similar variables for
	private int quickComparisons;
	...
	// method: swap
	// description: This method will swap two elements in array numbers and then
	// repaint the panel.
	// Your code should call this method whenever you want to swapt two elements.
	// parameters: int i and int j - the two indexes that of the elements that will
	// be swapped.
	...
	// method: BubbleSort
	// description: This method will sort the integers in array numbers using the
	// bubble sort algorithm. As it sorts the
	// array it will set the two instance fields compareIndex1 and compareIndex2 to
	...
	private void bubbleSort() {
		for (int j = 0; j < numbers.length - 1; j++) {
			for (int i = 0; i < numbers.length - j - 1; i++) {
				...
				this.paintImmediately(getVisibleRect()); // repaint the panel. Your code should repaint the panel after
															// each
															// comparison and swap.
	...
	private void combSort() {
		...
		double gapShrink = 1.3;
		boolean sorted = false;

		while (!sorted) {
			gap = (int) Math.floor(gap / gapShrink);
			if (gap <= 1) {
				gap = 1;
				sorted = true;
			}
	...
	private void merge(int left, int middle, int right) {
		...
		System.arraycopy(numbers, left, tempL, 0, length1);
		System.arraycopy(numbers, middle + 1, tempR, 0, length2);
		...
		while (i < length1 && j < length2) {
			...
			if (tempL[i] <= tempR[j]) {
				numbers[index] = tempL[i];
				i++;
			} else {

				numbers[index] = tempR[j];
				j++;
			}
			index++;
		}
	...
	private void quickSort(int left, int right) {
		while (right - left >= 10) {
			...
			int pi = medianPartition(left, right);
	...
	private int medianPartition(int left, int right) {
	...
	// method: resizeArray
	// description: This function is used by the createArray. It resizes the array
	// as the numbers are read in from the
	// text file by either doubling the size if more space is needed or shrinking
	// the size of the array
	// after all numbers have been added. You shouldn't modify or call this
	// function.
	...
	// method: createArray
	// description: This function will add all of the numbers from the text file
	// FILE_NAME into the array numbers.
	...
	// method: toString
	// description: Prints the contents of the numbers array. You could use this
	// function if you need to debug your code.
	...
	// description: This function paints all contents onto the panel. This is the
	// only function where you can draw. It will
	...
	public void paintComponent(Graphics g) {
		...
		// finish is initially false. It is set to true once all sorting algorithms have
		// completed.
		// Once all of the sorting algorithms have completed your code should display
		// the total
		...
		if (!finish) {
			...
			for (int i = 0; i < numbers.length; i++) { // iterate through the array
				if (compareIndex1 == i || compareIndex2 == i) // set the color to red if current element is one
					...
					g2.setColor(Color.BLACK); // otherwise set it to Cyan. You can off course change this.

				// Draw a rectangle for the current number. Height is based on the number's
				// value.
				g2.fillRect(i * WIDTH / numbers.length, HEIGHT - numbers[i], WIDTH / numbers.length, numbers[i]);

				// This draws a outline around the rectangle.
			...
			// This code currently writes out how many comparisons bubble sort required. You
			// will update it
			// by adding counts for each of the sorting algorithms that you implement.
			g2.setColor(Color.GREEN);
			g2.setFont(new Font("Arial", Font.BOLD, 35));

			g2.drawString("BubbleSort", WIDTH / 2 - 227, HEIGHT / 2 - 200);
			g2.drawString("Comparisons = " + String.valueOf(bubbleComparisons), WIDTH / 2 - 20, HEIGHT / 2 - 200);

			g2.drawString("SelectionSort", WIDTH / 2 - 250, HEIGHT / 2 - 100);
			g2.drawString("Comparisons = " + String.valueOf(selectionComparisons), WIDTH / 2 - 20, HEIGHT / 2 - 100);

			g2.drawString("InsertionSort", WIDTH / 2 - 250, HEIGHT / 2);
			g2.drawString("Comparisons = " + String.valueOf(insertionComparisons), WIDTH / 2 - 20, HEIGHT / 2);

			g2.drawString("CombSort", WIDTH / 2 - 227, HEIGHT / 2 + 100);
			g2.drawString("Comparisons = " + String.valueOf(combComparisons), WIDTH / 2 - 20, HEIGHT / 2 + 100);

			g2.drawString("MergeSort", WIDTH / 2 - 227, HEIGHT / 2 + 200);
			g2.drawString("Comparisons = " + String.valueOf(mergeComparisons), WIDTH / 2 - 20, HEIGHT / 2 + 200);

			g2.drawString("QuickSort", WIDTH / 2 - 227, HEIGHT / 2 + 300);
			g2.drawString("Comparisons = " + String.valueOf(quickComparisons), WIDTH / 2 - 20, HEIGHT / 2 + 300);
		}

		if (!start) {
			start = !start;

			// These next four lines of code contain everything that is necessary to call
			// the bubble sort process.
			// You'll follow the same process for each of the other sorting algorithms that
			// you write.
...
//			System.out.println("BubbleSort: " + (endTime - startTime) / 1000000 + " milliseconds");
...
//			selectionSort();
//			endTime = System.nanoTime();
//			System.out.println("SelectionSort: " + (endTime - startTime) / 1000000 + " milliseconds");
...
//			endTime = System.nanoTime();
//			System.out.println("InsertionSort: " + (endTime - startTime) / 1000000 + " milliseconds");
//			insertionComparisons = comparisons;
//
//			createArray();
//			sortingName = "CombSort";
//			startTime = System.nanoTime();
//			combSort();
//			endTime = System.nanoTime();
//			System.out.println("CombSort: " + (endTime - startTime) / 1000000 + " milliseconds");
//			combComparisons = comparisons;
//
//			createArray();
//			sortingName = "MergeSort";
//			startTime = System.nanoTime();
//			mergeSort(0, numbers.length - 1);
//			endTime = System.nanoTime();
//			System.out.println("MergeSort: " + (endTime - startTime) / 1000000 + " milliseconds");
//			mergeComparisons = comparisons;
			...
			endTime = System.nanoTime();
			System.out.println("Quicksort: " + (endTime - startTime) / 1000000 + " milliseconds");
			quickComparisons = comparisons;

			System.out.println("For an array of " + numbers.length + " elements");

			// Insert calls to your other sorting algorithms. Remember to recreate your
